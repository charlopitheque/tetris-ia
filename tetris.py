...
class Tetris:
    ...
    def get_next_rotation(self):
        ...
        if self.current_rotation == 90:
            return 180
        if self.current_rotation == 180:
            return 270
        if self.current_rotation == 270:
            return 0

    def player_play(self, render=False, render_delay=None):
        '''Makes a play given a position and a rotation, returning the reward and if the game is over'''
        ...
        if(not self.game_over):
            ...
            self.current_rotation = 0
            while not self._check_collision(self._get_rotated_piece(), self.current_pos) and self.current_pos[1] > -1:

                key = cv2.waitKey(300)
                '''left'''
                if key == 81 and self.current_pos[0] > -1:
                ...
                if key == 84:
                    ...
                    if self._check_collision(self._get_rotated_piece(), self.current_pos):
                        self.current_pos[1] -= 1
                    if self._check_collision(self._get_rotated_piece(), self.current_pos) and self.current_pos[1] < 0 or not self._check_collision(self._get_rotated_piece(), self.current_pos) and self.current_pos[1] < 0:
                        self.game_over = True
                        self.current_pos[0] = 0
                        self.current_pos[1] = 0

                '''right'''
                if key == 83 and self.current_pos[0] < self.BOARD_WIDTH:
                    self.current_pos[0] += 1
                    if self._check_collision(self._get_rotated_piece(), self.current_pos):
                        self.current_pos[0] -= 1
                '''space'''
        ...
        self.board = self._add_piece_to_board(self._get_rotated_piece(), self.current_pos)
        lines_cleared, self.board = self._clear_lines(self.board)
        sum_height, max_height, min_height = self._height(self.board)
        score = (1 + (lines_cleared ** 2) * Tetris.BOARD_WIDTH)  # * (max_height / self.BOARD_HEIGHT)
        self.score += score
        best_action = tuple([self.current_pos[0], self.current_rotation])
        ...
        return score, self.game_over, best_action
